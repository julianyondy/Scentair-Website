...
type Point = { icon: React.ReactNode; value: string; label: React.ReactNode };
...
const BannerBlock: React.FC<{
  ...
  if (isVideoSection) {
    return (
      /* ↓↓ tightened vertical space here ↓↓ */
      <section className="relative w-full pt-2 pb-8 md:pt-4 md:pb-10 bg-white">
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative flex flex-col items-center">
            ...
            <h2 className="text-slate-900 text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-center mb-3 z-10">
              {headline}
            </h2>
            
            {/* Video centered and larger */}
            {/* ↓↓ slight negative margin to pull video up ↓↓ */
}
            <div className="relative w-full max-w-6xl mx-auto -mt-2 md:-mt-3">
              ...
              {points.map((p, i) => (
                ...
                >
                  ...
                  <div className="w-56 h-56 rounded-full bg-primary flex flex-col items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg p-6">
                    ...
                    <div className="text-[#0c4384] font-semibold text-lg text-center mt-3 px-3">
                      {p.label}
                    </div>
                  </div>
                </div>
...
export const WhyScentSection: React.FC = () => {
  const Header = () => (
    /* ↓↓ tightened header bottom padding ↓↓ */
    <section className="pt-8 pb-3 md:pt-12 md:pb-5">
  ...
  const emotionalPoints: Point[] = [
    { icon: <Brain size={32} className="text-white" />, value: '63%',  label: <><span className="text-lg md:text-xl font-bold">Improvement in mood</span> when exposed to a pleasing scent</> },
    { icon: <Smile size={32} className="text-white" />, value: '75%',  label: <><span className="text-lg md:text-xl font-bold">Our emotions are generated</span> by what we smell, not what we see</> },
  ...
  return (
    <div className="bg-white text-slate-900">
      ...
      <BannerBlock
        ...
        isReversed={true}
        description={<span className="font-semibold">A study by css datatelligence compared shoppers in a ScentAir scented and an unscented environment. In the scented space:</span>}
      />
